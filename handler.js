...
let name = command
if (name) {
  global.db.data.stats[name] = global.db.data.stats[name] || 0
  global.db.data.stats[name] += 1
}
        ...
        try {
            ...
            const chatDefaults = {
                ...
                delete: false, 
                antiLink: false, viewonce: false, antiToxic: false, simi: false, autogpt: false, autoSticker: false, plus: false, plusTime: false, nsfw: false, menu: true, rpgs: true, expired: 0
            }
        ...
        const isROwner = [conn.decodeJid(global.conn.user.id), ...global.fahriown.map(([number]) => number)].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
        const isOwner = isROwner || m.fromMe
        const isMods = isOwner || global.mods.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
        const isPrems = isROwner || global.db.data.users[m.sender].plusTime > 0
    ...
    if (chat?.isBanned) {
        if (!['owner-unbanchat.js'].includes(name)) continue
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (plugin.plus && !isPrems) { fail('plus', m, this); continue }
...
global.dfail = (type, m, conn) => {
    let msg = {
        rowner: 'Khusus developer Victoria 🎧',
        owner: 'Hanya untuk owner Victoria 🎸',
        mods: 'Bagian moderator 🎯',
        plus: 'Fitur ini untuk pengguna Plus ✨',
        group: 'Gunakan di dalam grup 👥',
        private: 'Gunakan di chat pribadi 💬',
        admin: 'Khusus admin grup 🛡️',
        botAdmin: 'Victoria perlu jadi admin dulu ⚙️',
