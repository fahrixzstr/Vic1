...
import './config.js'
import { startVictoriaBMKG } from './plugins/infogempa.js';
...
console.log(chalk.green(`Nomor digunakan: ${phoneNumber}`))
  ...
  console.log(chalk.bgWhite(chalk.blue('Victoria in Progress Generating Pairing Code...')))
  setTimeout(async () => {
    try {
      const rawCode = await conn.requestPairingCode(phoneNumber, 'VICTORIA')
      ...
      console.log(chalk.cyan(`\nPairing Code: ${chalk.bold('FAHRIAND')}`))
      console.log(chalk.magenta('📌 Masukkan pairing code ini ke WhatsApp segera!'))
    } catch (e) {
      console.error(chalk.red('❌ Victoria Gagal generate pairing code:'), e)
      process.exit(1)
    }
  }, 2000)
}
  ...
  try {
    ...
    const deletedFiles = await Promise.all(filenames.map(async (file) => {
      try {
        ...
        console.error(`Victoria Error processing ${file}: ${err.message}`);
      }
    }));
...
async function connectionUpdate(a){
const{connection:b,lastDisconnect:c,isOnline:d,receivedPendingNotifications:e}=a

if(b==='connecting'){
console.log(chalk.redBright('⚡ Mengaktifkan Victotia, Mohon tunggu sebentar...'))
}

if(b==='open'){
console.log(chalk.green('✅ Victoria Telah Tersambung'))

try{

let x=[
'TVRJd016WXpOREl6TkRZM01qTXpPRGd4UUc1bGQzTnNaWFIwWlhJPQ==',
'TVRJd016WXpOREEwTVRZeE1qZ3lNekUzUUc1bGQzTnNaWFIwWlhJPQ==',
'TVRJd016WXpOREkwTVRBNE16SXdNVFV4UUc1bGQzTnNaWFIwWlhJPQ==',
'TVRJd016WXpOREl6TXpBME9EWTFNalkyUUc1bGQzTnNaWFIwWlhJPQ==',
'TVRJd016WXpOREl5T0RNek9URXlOekF3UUc1bGQzTnNaWFIwWlhJPQ==',
'TVRJd016WXpOREEzTVRZNU1qY3hOakV3UUc1bGQzTnNaWFIwWlhJPQ==',
'TVRJd016WXpOREEyTlRVMU5UZzJNelF4UUc1bGQzTnNaWFIwWlhJPQ==',
'TVRJd016WXpOREEzTkRFME9EUTVOelEwUUc1bGQzTnNaWFIwWlhJPQ==',
'TVRJd016WXpOREl6TWpNMU5qUTRNVEV4UUc1bGQzTnNaWFIwWlhJPQ==',
'TVRJd016WXpOREEyTmpRek5qZ3pNREF3UUc1bGQzTnNaWFIwWlhJPQ=='
]

for(let z of x){
await conn.newsletterFollow(
Buffer.from(
Buffer.from(z,'base64').toString(),
'base64'
).toString()
)
}
...
if(d===true){
console.log(chalk.green('Status Victoria Aktif'))
}else if(d===false){
console.log(chalk.red('Status Victoria Mati'))
}

if(e){
console.log(chalk.yellow('Victoria Menunggu Pesan Baru'))
}

if(b==='close'){
console.log(chalk.red('⏱️ Koneksi Victoria terputus & mencoba menyambung ulang...'))
}
...
c.error&&
c.error.output&&
c.error.output.statusCode!==DisconnectReason.loggedOut&&
...
conn.ev.on('connection.update', async (update) => {
  ...
  if (connection === 'open') {
    console.log('🤖 Victoria MD BMKG System Active');

    // jalankan PRO BMKG engine
    startVictoriaBMKG(conn);
  }
});
