...
const JACKPOT_POOLS = [
  ...
  },
  {
    ...
    rewards: [
      ...
      { type: "plus_7d", min: 1, max: 1, weight: 10 },
  ...
  },
  {
    ...
    rewards: [
      ...
      { type: "plus_30d", min: 1, max: 1, weight: 10 },
  ...
  },
  {
    ...
    rewards: [
      ...
      { type: "plus_30d", min: 1, max: 1, weight: 15 },
...
function applyJackpotReward(db, fischUser, sender, result) {
  ...
  switch (type) {
    ...
    case "plus_7d":
    case "plus_30d": {
      const days = type === "plus_7d" ? 7 : 30;
      const cleanJid = sender.split("@")[0];
      const user = db.db.data.users[cleanJid];
      if (user) {
        const now = Date.now();
        const existingExpiry = user.plusExpiry
          ? new Date(user.plusExpiry).getTime()
          : 0;
        const start = existingExpiry > now ? existingExpiry : now;
        user.plusExpiry = new Date(start + days * 86400000).toISOString();
        user.isplus = true;
        db.markDirty("users");
      }
      applied = { desc: `plus ${days} Hari!`, type };
      break;
    }
    case "partner_7d":
    case "partner_30d": {
