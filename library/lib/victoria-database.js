...
import { logger } from "./victoria-logger.js";
...
class Database {
  ...
  async init() {
    try {
      ...
      const currentplus = config.energi?.plus ?? 100;
      const lastDefault = this.db.data.settings._lastEnergiDefault;
      const lastplus = this.db.data.settings._lastEnergiplus;

      if (lastDefault !== currentDefault || lastplus !== currentplus) {
        ...
        this.db.data.settings._lastEnergiplus = currentplus;
        this.markDirty("users");
        this.markDirty("settings");
        if (synced > 0) {
          logger.info(
            "database",
            `energi sync ${synced} user di-update (default: ${currentDefault}, plus: ${currentplus})`,
          );
        }
      }
  ...
  setUser(jid, data = {}) {
    ...
    this.db.data.users[cleanJid] = {
      ...
      isplus: data.isplus ?? existing.isplus ?? false,
  ...
  updateEnergi(jid, amount) {
    ...
    try {
      ...
      const plusEnergi = config.energi?.plus ?? -1;
      const isOwnerUser = config.isOwner(jid);
      const isplusUser = config.isplus(jid);
      if (isOwnerUser && ownerEnergi === -1) return -1;
      if (isplusUser && plusEnergi === -1) return -1;
    } catch {}
  ...
  resetAllEnergi(defaultEnergi = 25, plusEnergi = -1) {
    let count = 0;
    for (const jid of Object.keys(this.db.data.users)) {
      const user = this.db.data.users[jid];
      user.energi = user.isplus ? plusEnergi : defaultEnergi;
      count++;
    }
    this.markDirty("users");
    return count;
