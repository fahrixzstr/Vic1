...
import { getDatabase } from "./victoria-database.js";
...
footer: Powered by victoria AI (opsional)
[/RICH:CODE]
...
[RICH:LIST]
title: Info Bot
rows: Nama | victoria AI;; Versi | 2.4.5;; Developer | fhri
footer: © victoria AI (opsional)
[/RICH:LIST]

5. STICKER (saat user minta sticker, atau untuk ekspresi emosi):
[RICH:STICKER]
url: https://iili.io/BPBdFuj.md.jpg
packname: victoria AI (opsional)
  ...
  try {
    ...
    if (rich.type === "STICKER") {
      ...
      await sock.sendImageAsSticker(jid, stickerInput, quoted, {
        packname: packname || config.bot?.name || "victoria AI",
...
async function executeAction(action, m, sock) {
  ...
  switch (action.type) {
    case "KICK": {
      if (!m.isGroup && config.autoAIGroupOnly) return [{ ok: false, msg: "Bukan di grup" }];
    ...
    case "STICKER": {
      ...
      await sock.sendMessage(
        m.chat,
        {
          ...
          packname: config.bot?.name || "victoria",
          author: "AutoAI",
        },
        { quoted: m },
      );
...
async function handleAutoAI(m, sock) {
  ...
  if (!db.db.data.autoai_private) db.db.data.autoai_private = {};
  ...
  // =========================
  // 1. GET CONFIG PER CHAT
  // =========================
  if (isGroup) {
    autoai = db.db.data.autoai[m.chat];
  } else {
    autoai = db.db.data.autoai_private[m.chat];
  }
  ...
  // 🌍 global fallback
    ...
    autoai = {
      ...
      enableCommands: globalCfg.enableCommands || false,
    };
  }

  if (!autoai) return false;
  // =========================
  // 3. BOT IDENTITY
  // =========================
  ...
  if (m.isCommand && !m.isOwner) {
    if (autoai.enableCommands === false) return false;
    return !!autoai;
  }

  // =========================
  // lanjut proses AI (lanjutan kamu)
  // =========================
  ...
  const isMentioned = m.mentionedJid?.some((jid) => {
    if (!jid) return false;
    if (jid === botJid) return !!autoai;
    if (jid === botLid) return !!autoai;
    if (jid === botFullId) return !!autoai;
    if (jid.includes(botNumber)) return !!autoai;
    const jidUser = jid.split("@")[0]?.split(":")[0];
    if (jidUser && jidUser === botNumber) return !!autoai;
    return false;
  });
