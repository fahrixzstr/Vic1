...
import {
  ...
  cacheLidJid,
  resolveFromSock,
} from "./victoria-lid.js";
import fsc from "fs";
import axios from "axios";
import { getDatabase } from "./victoria-database.js";
import { saluranCtx } from "./victoria-context.js";
import { getAssetBuffer } from "./victoria-asset-manager.js";
 ...
 * @property {boolean} isplus - Apakah pengirim adalah plus user
...
function createContextInfo(jid, text, title = "", body = "", thumbnail = null) {
  ...
  if (jid && text) {
    ...
    contextInfo.stanzaId = "victoriaAI" + Date.now();
  }
...
async function serialize(sock, msg, store = {}) {
  ...
  m.isplus = m.isNewsletter || m.fromMe ? true : isplus(m.sender);
  ...
  m.botName = sock.user?.name || config.bot?.name || "victoria-AI";
  m.messageId = m.id;
  m.chatId = m.chat;
  m.senderId = m.sender;
    ...
    if (replyVariant === 2) {
      ...
      return sock.sendMessage(
        m.chat,
        {
          ...
          jpegThumbnail: await sharp(getAssetBuffer("victoria2"))
            .resize(300, 300)
            .toBuffer(),
          caption: text,
          ...defaultOptions,
      ...
      const sss = getAssetBuffer("victoria3");
      return sock.sendMessage(
        m.chat,
        {
          video: getAssetBuffer("victoria-mp4"),
          caption: text,
          gifPlayback: true,
          contextInfo: {
            ...contextInfo,
      ...
      const thumbnail = getAssetBuffer("victoria");
      ...
      return sock.sendPreview(
        m.chat,
        {
          ...
          title: config.bot?.name || "victoria-AI",
          description:
        ...
        },
        {
          ...
          contextInfo: {
            ...
            forwardedNewsletterMessageInfo: {
              ...
              newsletterName: config.saluran?.name || config.bot?.name || "victoria-AI",
