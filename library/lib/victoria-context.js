...
let winnerThumbBuffer = null;
  ...
  const keys = [
    ["victoria-games", (buf) => { gameThumbBuffer = buf; }],
    ["victoria-rpg", (buf) => { rpgThumbBuffer = buf; }],
    ["victoria-winner", (buf) => { winnerThumbBuffer = buf; }],
  ];
...
function _saluranCtx() {
  ...
  const saluranName = config.saluran?.name || config.bot?.name || "victoria-AI";
  return {
    forwardingScore: 9,
    isForwarded: true,
...
function getRpgContextInfo(title, body) {
  ...
  if (title || body) {
    base.externalAdReply = {
      title: title || config.bot?.name || "victoria RPG",
      body: body || "",
...
function createFakeQuoted(botName = "victoria-AI", verified = true) {
  return {
    key: {
      fromMe: false,
