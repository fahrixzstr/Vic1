...
function isJadibotplus(jadibotId, jid) {
    const db = loadJadibotDb(jadibotId)
    const senderNum = jid?.replace(/[^0-9]/g, '') || ''
    return db.pluss.some(p => {
        const premNum = typeof p === 'string' ? p.replace(/[^0-9]/g, '') : p.jid?.replace(/[^0-9]/g, '') || ''
        return premNum === senderNum
    })
}

function addJadibotplus(jadibotId, jid, expiry = null) {
    const db = loadJadibotDb(jadibotId)
    const num = jid.replace(/[^0-9]/g, '')
    const existing = db.pluss.find(p => {
        const premNum = typeof p === 'string' ? p : p.jid?.replace(/[^0-9]/g, '') || ''
        return premNum === num
    })
    
    if (!existing) {
        db.pluss.push({ jid: num, expiry: expiry || null })
        saveJadibotDb(jadibotId)
        return true
    }
    return false
...
function removeJadibotplus(jadibotId, jid) {
    const db = loadJadibotDb(jadibotId)
    const num = jid.replace(/[^0-9]/g, '')
    const idx = db.pluss.findIndex(p => {
        const premNum = typeof p === 'string' ? p : p.jid?.replace(/[^0-9]/g, '') || ''
        return premNum === num
    })
    
    if (idx !== -1) {
        db.pluss.splice(idx, 1)
        saveJadibotDb(jadibotId)
        return true
    }
    return false
...
function getJadibotpluss(jadibotId) {
    const db = loadJadibotDb(jadibotId)
    return db.pluss
}
...
export { loadJadibotDb, saveJadibotDb, isJadibotOwner, addJadibotOwner, removeJadibotOwner, getJadibotOwners, isJadibotplus, addJadibotplus, removeJadibotplus, getJadibotpluss, getJadibotSetting, setJadibotSetting, getJadibotUser, setJadibotUser, getJadibotGroup, setJadibotGroup, getAllJadibotData }