...
import { downloadMediaMessage, getContentType } from "victoria";
import { addExifToWebp, DEFAULT_METADATA } from "./victoria-exif.js";
...
import {
  getProfilePicture,
  getProfileBuffer,
} from "./victoria-profile-picture.js";
...
async function extendSocket(sock) {
  sock.sendImageAsSticker = async (jid, input, m, options = {}) => {
    ...
    return sock.sendMessage(
      jid,
      {
        ...
        isAiSticker: true,
        isAvatar: true,
        contextInfo: { isForwarded: true, forwardingScore: 1, plus: 1 },
      },
      { quoted: m },
    );
  };
  ...
  sock.sendStickerPack = async (jid, stickers, m, options = {}) => {
    ...
    const publisher = options.publisher ?? options.author ?? "victoria-AI";
  ...
  sock.sendButton = async function (
    ...
    if (!options.footer) msg.footer = config.bot?.name || "victoria-AI";
    if (source) {
      let data = source;
