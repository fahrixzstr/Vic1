...
import { addJadibotOwner } from "./victoria-jadibot-database.js";
import { extendSocket } from "./victoria-socket.js";
  ...
  if (usePairing && !state.creds?.registered) {
    try {
      ...
      if (m && m.chat) {
        let thumbnail = null;
        try {
          if (!!getAssetBuffer("victoria2")) {
            thumbnail = getAssetBuffer("victoria2");
          }
        } catch {}
